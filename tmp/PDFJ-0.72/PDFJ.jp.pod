...
=item フォント（PDFJ::AFont、PDFJ::JFont）
...
$encodingはエンコーディングで、次のいずれかの定義済みエンコーディング名を指定する。省略すると、欧文フォントに対しては'WinAnsiEncoding'、日本語フォントに対しては'90ms-RKSJ-H'が使われる。
...
このように日本語フォントと欧文フォントを組み合わせたフォントオブジェクトをテキストに対して指定すると、テキスト中の日本語部分と欧文部分に対してそれぞれのフォントが自動的に切り替えて適用される。これにより、欧文部分についてはプロポーショナルな表示となる。
...
引数にはハッシュリストの形で次のものを与える。sizeとlinefeedとalignは必須。他はオプション。
  ...
  float => ブロックのbreakで、b:先頭、e:末尾、h:可能ならその位置、に移動
...
labeltextでテキストオブジェクトが指定されると、ラベルとして先頭行の前に表示される。ラベルと本文の間にはlabelskipだけの間隔が取られる。
...
($x,$y)が中心、$rが半径の円が描かれる。$arcareaは四半円指定。$styleは図形スタイルオブジェクトで、省略可能。$specは次の描画指定。
...
=head2 PDFJ::JFontクラスのメソッド

  PDFJ::JFont->new_std($docobj, $basefont, $encoding, $afontobj)
  PDFJ::JFont->new_ttf($docobj, $ttffile, $encoding, $afontobj)
